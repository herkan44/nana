  ...
  radios.forEach(radio => {
    radio.addEventListener('change', () => {
      ...
      if (radio.value === 'bank' || radio.value === 'ewallet') {
        html += '<img class="qris" src="https://files.catbox.moe/k8x25m.webp" />';
        html += `<p style="font-weight:600;color:#fff;">QR UNTUK SEMUA JENIS ${radio.value.toUpperCase()}</p>`;
        html += `<button onclick="window.location.href='https://files.catbox.moe/k8x25m.webp'">Download Qris</button>`;
        html += getInputSection();
      } else if (radio.value === 'dana') {
        html += '<div class="dana-info">GOPAY - SCAN QRIS -  </div>';
